...
void osmo_stream_cli_destroy(struct osmo_stream_cli *cli)
{
	...
	OSMO_ASSERT(!stream_cli_close(cli)); /* BUG IN APP, TRYING TO DESTROY() WHILE ALREADY IN DESTROY()*/
	osmo_timer_del(&cli->timer);
...
int osmo_stream_cli_set_priority(struct osmo_stream_cli *cli, int sk_prio)
{
	...
	if ((fd = osmo_stream_cli_get_fd(cli)) < 0) { /* Shouldn't happen... */
		LOGSCLI(cli, LOGL_ERROR, "set_priority(%u): failed obtaining socket\n", cli->sk_prio);
		return -EBADFD;
	}
	if ((rc = osmo_sock_set_priority(fd, cli->sk_prio)) < 0)
		LOGSCLI(cli, LOGL_ERROR, "set_priority(%u): failed setsockopt err=%d\n",
			cli->sk_prio, errno);
	return rc;
}
